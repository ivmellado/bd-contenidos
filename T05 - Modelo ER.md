...
2. **Fase 2. Lógica:** Traducimos sistemáticamente el **Diagrama ER** a a un conjunto de relaciones, siguiendo unas reglas claras. El resultado es un **esquema relacional**.
...
>[!tip] De la Idea al Diagrama - Modelando con BigER y Crow's Foot
> 
  > |                                                                                                         | Textual en BigER                                                                                                                                                                        | Notación Crow's foot en BigER                                                                                                   |
| ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
...
#### 5.4.1 Caso 1: Para modelar atributos multivalor complejos

Imagina un `CURSO` que tiene varias `EDICIONES` cada año. Podríamos tener un atributo multivalor `{Edicion}`. 

| BigER                                                                                            | Crow's foot                                                |
| ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------- |
| ```entity``` Curso {<br>idCurso ```key```<br>nombreCurso<br>fechaInicio<br>fechaFin<br>aula<br>} | ![](resources/BD%20-%20BigER%20Entidad%20Curso%20Foot.png) |
Pero, ¿y si cada edición tiene sus propios atributos, como `FechaInicio`, `FechaFin` y `Aula`?. Un simple atributo multivalor no puede guardar toda esa estructura.

**Solución**: Convertimos `EDICION` en una entidad débil que depende de `CURSO`. Su clave parcial podría ser `FechaInicio`. Así, podemos almacenar toda la información de cada edición de forma ordenada.

| BigER                                                                                                                                                                                                                                                                   | Crow's foot                                                        |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| ```entity``` Curso {<br>idCurso ```key```<br>nombreCurso<br>fechaInicio<br>fechaFin<br>aula<br>}<br>==weak entity== Edicion {<br>fechaInicio ==partial-key==<br>fechaFin<br>aula<br>}<br>==weak relationship== tiene {<br>Curso[```1..1```] -> Edicion[```0..N```]<br>} | ![](resources/BD%20-%20BigER%20Debil%20Edicion%20Crows%20Foot.png) |
#### 5.4.2 Caso 2: Para gestionar atributos multivaluados en una relación M:N o M:N con repetición de relaciones entre instancias.
...
Por ejemplo, un `PROFESOR` imparte una `ASIGNATURA`. Esta es una relación M:N. Ahora queremos registrar en qué **año académico (`curso`)** se impartió cada docencia. El problema es que un profesor puede impartir la asignatura de "BD" en múltiples cursos (08/09, 22/23, 23/24). Es decir, el mismo par (Profesor, Asignatura) se puede repetir por lo que el atributo curso realmente sería multivaluado.
...
| BigER                                                                                                                                                                                                                                                                                                                                                                                                            | Crow's foot                                                         |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| ```entity``` PROFESOR {<br>nif ```key```<br>nombre<br>apellido1<br>apellido2<br>}<br>```entity``` ASIGNATURA {<br>cod_asignatura ```key```<br>nombre<br>}<br><br>==weak entity== DOCENCIA {<br>curso ==partial-key==<br>}<br><br>==weak relationship== imparte {<br>PROFESOR[```1..1```] -> DOCENCIA[```1..N```]<br>}<br><br>==relationship== relativaA {<br>DOCENCIA[```1..N```] -> ASIGNATURA[```1..1```]<br>} | ![](resources/BD%20-%20BigER%20Debil%20Docencia%20Crows%20Foot.png) |
